...
import { DocxCommentTool } from '@/main/tools/OfficeTool';
...
export class ReviewAgent extends BaseAgent {
  ...
  async createAgent(store?: BaseStore, model?: BaseChatModel) {
    ...
    const prompt = fs.readFileSync(
      path.join(getAssetsPath(), 'prompts', `review-${language}.md`),
      'utf-8',
    );
    const officeTools = await toolsManager.buildTools(['read-docx']);
